...
async function dismissConsent(page) {
  ...
  // text-match fallback for custom CMPs (rwe.com: "Welcome to RWE" dialog with
  // Accept all / Deny all / Save Settings buttons and no accept-ish id/class).
  await page.evaluate(() => {
    const btns = [...document.querySelectorAll('button, [role="button"], a')];
    const hit = btns.find((b) => /^\s*(accept all|allow all|agree to all|alle akzeptieren)\s*$/i.test(b.textContent || ''));
    if (hit) hit.click();
  }).catch(() => {});
