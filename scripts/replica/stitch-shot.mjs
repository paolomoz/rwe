...
async function main() {
  ...
  try {
    ...
    // INSTRUMENT FIX (rwe replica run, 2026-08-26; source-fidelity-gate.md
    // § Script adaptations — documented instrument-bug fix): the CSS freeze
    // above stabilizes CSS animations only. It does NOT stop (a) <video>
    // playback — autoplaying teaser videos capture an arbitrary frame per
    // run, so the same page never pixel-matches itself; (b) JS-timer
    ...
    // clear all pending timeouts/intervals so timer-driven UI stops
    // mutating mid-capture. Runs AFTER settle, so lazyload is done.
    // Flagged for upstreaming into the plugin's stitch-shot.mjs.
    await page.evaluate(async () => {
    ...
    // INSTRUMENT FIX (rwe replica run, 2026-08-26; source-fidelity-gate.md
    // § Script adaptations): --fullpage bypasses the chunk loop and takes a
    // single fullPage screenshot AFTER the settle + freeze passes. On
    // rwe.com the chunked capture reproducibly corrupts the last chunk
    // (the site's scroll-state compact header bakes into the seam and the
    // footer content wraps horizontally by half the viewport — compositor
    // artifact on the clamped final scroll). Rule 4's lazy-gray-placeholder
    // trap does not bite AFTER a full settle pass (all imgs decoded) —
    // verify by eyeball per capture. Use SYMMETRICALLY on both sides.
    // Flagged for upstreaming.
    if (opts.fullpage) {
      // Part of the same instrument fix: sites that REMOVE their entrance-
      // animation class when a section scrolls out of view (rwe.com:
      // `.bottom-to-top-animation:not(.animate){opacity:0}`) render every
      // below-fold section blank in a fullPage shot taken from scrollTop 0.
      // Force the settled final state (opacity 1, no entrance animation) on
      // animation-classed elements — injected on BOTH sides, so the
      // instrument stays symmetric (a prototype without these classes is
      // unaffected).
      await page.addStyleTag({ content: '[class*="-animation"],[class*="wobble"]{opacity:1!important;animation:none!important}' });
      // Lock iframe heights: responsive embeds (rwe.com: euroland share
      // ticker) re-measure themselves when fullPage rendering resizes the
      // viewport to the document height — the embed grew +125px mid-capture,
      // shifting all content below it and cropping the page tail. Freeze
      // each iframe at its settled height before the shot (symmetric).
      await page.evaluate(() => {
      ...
      console.log(`fullpage ${out}: ${img.width}x${img.height} (settle+freeze hardening applied, chunk loop bypassed)`);
      return;
    }
